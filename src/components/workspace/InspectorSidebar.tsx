...
import { VariableJourney } from "@/components/inspector/VariableJourney";
import { FunctionJourney } from "@/components/inspector/FunctionJourney";
...
export function InspectorSidebar() {
  const { step, program, index, state, selection, select, hover, setHover } = useExecution();
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3">
        <AnimatePresence mode="wait">
          {activeTab === "variables" ? (
            ...
            >
              <div className="flex items-center justify-between border-b border-border/50 pb-2">
                <span className="font-sans text-xs font-semibold text-foreground">Step Explanation</span>
                <span className="rounded bg-cyan-500/10 px-2 py-0.5 text-[10px] text-cyan-300">
                  Line {step.line}
                </span>
              </div>
              <div className="rounded-xl border border-cyan-500/30 bg-cyan-500/10 p-4 space-y-3 font-sans">
                <div>
                  <div className="text-[11px] font-semibold uppercase tracking-wider text-cyan-400">What Happened?</div>
                  <div className="mt-1 text-xs leading-relaxed text-foreground">
                    {step.explanation?.summary || `Line ${step.line} executed successfully.`}
                  </div>
                </div>
                <div>
                  <div className="text-[11px] font-semibold uppercase tracking-wider text-purple-400">What Changed?</div>
                ...
                <div>
                  <div className="text-[11px] font-semibold uppercase tracking-wider text-emerald-400">What Happens Next?</div>
                  <div className="mt-1 text-xs leading-relaxed text-muted-foreground">
